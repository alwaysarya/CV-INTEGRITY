...
Real data from outputs/reports/*_quality_report.json + OOD detection
"""
...
def create_backdoor_page():
    ...
    def backdoor():
        ...
        with ui.column().classes('w-full px-8 py-8 gap-6'):
            ...
            page_title(ui, '🚨', 'Backdoor & Anomaly Detection', 'Trigger injection · Label flipping · Duplicate flooding · OOD insertion', '#EF4444')
            ...
            with ui.column().classes('w-full gap-4 mt-4'):
                ...
                with ui.row().classes('w-full gap-4 justify-center'):
                    for dataset_name, report in reports.items():
                        ...
                        with ui.card().classes('flex-1 p-5').style(f'border: 2px solid {color}; border-radius: 12px; max-width: 380px;'):
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                with ui.card().classes('w-full p-6').style('border: 1px solid rgba(59, 130, 246, 0.2); border-radius: 12px;'):
                    ...
                    if 'error' in scan:
                        ...
                        with ui.row().classes('w-full gap-4 mb-4'):
                            for label, value, c_color, icon in [
                                ...
                                with ui.card().classes('flex-1 p-3').style(f'border: 1px solid {c_color}40; border-radius: 8px; text-align: center;'):
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                for dataset_name, suspicious_list in all_suspicious.items():
                    ...
                    for sample in suspicious_list[:5]:
                        with ui.card().classes('w-full p-4 mb-2').style(f'border-left: 4px solid {sample["color"]}; border-radius: 12px;'):
            ...
            with ui.row().classes('w-full gap-3 justify-center mt-6'):
                ui.button('🔄 Refresh', on_click=lambda: ui.navigate.to('/backdoor')).classes('px-6 py-2 rounded-lg text-sm').style('background: linear-gradient(135deg, #EF4444, #DC2626); color: white;')
                
                ui.button('📊 Quality', on_click=lambda: ui.navigate.to('/quality')).classes('px-6 py-2 rounded-lg text-sm').style('background: rgba(16, 185, 129, 0.15); color: #10B981; border: 1px solid #10B981;')
