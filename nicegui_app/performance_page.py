...
def create_performance_page():
    ...
    def performance():
        ...
        with ui.column().classes('w-full px-8 py-8 gap-6'):
            ...
            page_title(ui, '⚡', 'Model Performance', 'YOLOv8n training metrics · Model comparison · Performance analysis', '#3B82F6')
            
            if not metrics:
                with ui.card().classes('w-full p-12').style('border: 2px dashed #3B82F6; border-radius: 12px;'):
                    ...
                    for model_name, model_metrics in metrics.items():
                        ...
                        with ui.card().classes('flex-1 p-6').style(f'border: 1px solid {color}40; border-radius: 12px; max-width: 400px;'):
                            with ui.row().classes('w-full items-center justify-between mb-4'):
                                ...
                                if is_best:
                                    ui.html('<div style="background: linear-gradient(135deg, #10B981, #059669); color: white; padding: 4px 12px; border-radius: 12px; font-size: 0.7rem; font-weight: 700;">🏆 BEST</div>')
                            ...
                            for metric_name, value in model_metrics.items():
                                ...
                                with ui.column().classes('w-full gap-1 mb-3'):
                                    ...
                                    ui.html(f'<div class="progress-bar"><div class="progress-fill" style="width: {min((value/max_val)*100, 100)}%; background: {color};"></div></div>')
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                for metric_name in ['precision', 'recall', 'mAP50', 'mAP50_95']:
                    with ui.card().classes('w-full p-5').style('border: 1px solid rgba(59, 130, 246, 0.15); border-radius: 12px;'):
                        ...
                        for model_name, model_metrics in metrics.items():
                            ...
                            with ui.row().classes('w-full items-center gap-3 py-2'):
                                ...
                                ui.html(f'<div class="progress-bar" style="flex: 1;"><div class="progress-fill" style="width: {width}%; background: {color};"></div></div>')
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                with ui.card().classes('w-full p-6').style('background: linear-gradient(135deg, rgba(16, 185, 129, 0.1), rgba(5, 150, 105, 0.05)); border: 2px solid #10B981; border-radius: 12px;'):
            ...
            with ui.row().classes('w-full gap-3 justify-center mt-6'):
                ui.button('🔄 Refresh', on_click=lambda: ui.navigate.to('/performance')).classes('px-6 py-2 rounded-lg text-sm').style('background: linear-gradient(135deg, #3B82F6, #2563EB); color: white;')
                ...
                ui.button('🧪 Robustness', on_click=lambda: ui.navigate.to('/robustness')).classes('px-6 py-2 rounded-lg text-sm').style('background: rgba(245, 158, 11, 0.15); color: #F59E0B; border: 1px solid #F59E0B;')
