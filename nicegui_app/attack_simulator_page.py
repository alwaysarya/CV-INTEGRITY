...
Reproducible attacks with before/after comparison
"""
...
def create_attack_simulator_page():
    ...
    def attack_simulator():
        ...
        with ui.column().classes('w-full px-8 py-8 gap-6'):
            ...
            page_title(ui, '⚔️', 'Attack Simulator', 'Reproducible attacks · Before/after comparison · Poisoning scenarios', '#EF4444')
            
            # Stats
            with ui.row().classes('w-full gap-4 justify-center'):
                for label, value, color, icon in [
                    ('Attack Types', '6', '#EF4444', 'bug_report'),
                    ...
                    if not image_options:
                        ...
                        with ui.row().classes('w-full gap-4 mt-6'):
                            with ui.column().classes('flex-1'):
                                ...
                                ui.image(f'/{rel_orig}').style('border-radius: 8px; width: 100%; height: 200px; object-fit: cover;')
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                with ui.row().classes('w-full gap-4 flex-wrap justify-center'):
                    for name, desc, color, icon in [
                        ...
                        with ui.card().classes('p-4').style(f'border: 2px solid {color}; border-radius: 12px; min-width: 160px;'):
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                with ui.row().classes('w-full gap-4 justify-center'):
                    for name, desc, color, icon in [
                        ...
                        with ui.card().classes('flex-1 p-5').style(f'border: 1px solid {color}60; border-radius: 12px; max-width: 350px; text-align: center;'):
                            ui.icon(icon).classes('text-4xl mb-2').style(f'color: {color};')
            ...
            with ui.column().classes('w-full gap-4 mt-6'):
                ...
                with ui.card().classes('w-full p-5').style('border: 1px solid rgba(239, 68, 68, 0.2); border-radius: 12px;'):
                        ...
                        with ui.row().classes('w-full gap-4 flex-wrap'):
                            for img_path in generated:
                                with ui.column().classes('items-center').style('flex: 0 0 calc(33.33% - 16px);'):
                                    ...
                                    ui.image(f'/{rel}').style('border-radius: 8px; width: 100%; height: 180px; object-fit: cover;')
                                    name = img_path.stem.split('_')[-1] if '_' in img_path.stem else img_path.stem
