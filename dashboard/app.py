...
def create_gauge_chart(value, title, max_val=100):
    color = "#00ff87" if value >= 80 else "#ffd700" if value >= 60 else "#ff4757"
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=value,
        title={'text': title, 'font': {'size': 16, 'color': '#fff', 'family': 'Arial Black'}},
        number={'suffix': "%", 'font': {'size': 32, 'color': color, 'family': 'Arial Black'}},
        gauge={
            'axis': {'range': [0, max_val], 'tickcolor': '#666', 'tickfont': {'color': '#888'}},
            ...
            'steps': [
                ...
                {'range': [50, 80], 'color': '#252540'},
                {'range': [80, 100], 'color': '#1a1a2e'}
            ],
        }
    ))
    fig.update_layout(
